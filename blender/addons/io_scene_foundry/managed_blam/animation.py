...
class AnimationTag(Tag):
    ...
    def write_ik_chains(self, ik_chains: list, bones: list, granny: bool = False):
        ...
        def calculate_ik_chain_order(chains):
            count = len(chains)
            result = []

            for chain_index, chain in enumerate(chains):
                antecedents = 1 << chain_index

                rank = count - 1 - chain_index

                result.append((rank, antecedents))
        ...
        chain_ranks_antecedents = calculate_ik_chain_order(valid_ik_chains)
    
        for chain, (rank, antecedents) in zip(valid_ik_chains, chain_ranks_antecedents):
            ...
            element.SelectField('rank').Data = rank
            element.SelectField('antecedents').Data = antecedents
    ...
    def _pose_bone_matches_transform_samples(
        self,
        blender_animation,
        pose_bone: bpy.types.PoseBone,
        fcurve_map,
        ...
        for sample_index, (sample_location, sample_rotation, sample_scale) in enumerate(samples):
            frame = blender_animation.frame_start + start_frame + sample_index
            matrix_cache = {}
            bone_matrix = self._sample_pose_bone_matrix_from_action(
                pose_bone,
                frame,
                fcurve_map,
                matrix_cache,
            )
            bone_location, bone_rotation, bone_scale = bone_matrix.decompose()
            bone_scale_value = (bone_scale.x + bone_scale.y + bone_scale.z) / 3.0

            if (bone_location - sample_location).length > location_tolerance:
                return False
            if self._rotation_angle_delta(bone_rotation, sample_rotation) > rotation_tolerance:
                return False
            if abs(float(bone_scale_value) - float(sample_scale)) > scale_tolerance:
                return False

        return True

    def _find_bone_matching_transform_samples(
        self,
        blender_animation,
        armature: bpy.types.Object,
        event,
        ...
        for preferred in preferred_names:
            preferred_bone = self._find_pose_bone_by_name(armature, preferred)
            if preferred_bone is not None and preferred_bone.name in matches:
                return preferred_bone.name
    ...
    def _reconstruct_ik_proxy_target_samples(
        self,
        blender_animation,
        armature: bpy.types.Object,
        event,
        ...
        if synthesize_leading_base_sample and reconstructed_samples and start_frame > 0:
            base_frame = blender_animation.frame_start
        ...
        return reconstructed_samples, prepended_base_sample
    ...
    def _import_ik_event_controls(self, tag_animation, blender_animation, blender_event, event, armature: bpy.types.Object, imported_actions: list):
        ...
        if effector_samples:
            reconstructed_proxy_marker_samples, prepended_base_sample = self._reconstruct_ik_proxy_target_samples(
                blender_animation,
                armature,
                event,
                effector_samples,
            ...
            if matching_bone_name:
                ...
                if chain is not None:
                    effector_pose_bone = self._find_pose_bone_by_name(armature, getattr(chain, "effector_node", ""))
                    if effector_pose_bone is not None:
                        effector_bone_name = effector_pose_bone.name

                if effector_bone_name is not None and matching_bone_name == effector_bone_name:
            ...
            tag_events.append(
                {
                    ...
                    "ik_game_marker_name": element.SelectField("proxy marker").GetStringData(),
    ...
    def _debug_print_applied_base_animation(
        ...
        reference_object_space_parent_targets: list,
    ) -> None:
        ...
        utils.print_info(
            ...
            f"reference targets={len(reference_object_space_parent_targets)}"
        )
    ...
    def _apply_object_space_reference_frame_corrections(
        self,
        tag_animation: Animation,
        animation_data,
        default_nodes: list[DefaultAnimationNode],
        ...
        for target_index, target_matrix, use_translation, use_rotation, use_scale in self._object_space_base_correction_targets(tag_animation, default_nodes):
            ...
            touched_node_indices.update(node_indices)
    ...
    def _build_animation(self, tag_animation: Animation, defaults, overlay_defaults, graph, shared_static_codec, resource_cache, animation_cache, all_tag_animations, final_frame_stack=None):
        ...
        if tag_animation.animation_type in (AnimationType.OVERLAY, AnimationType.REPLACEMENT):
            ...
            reference_object_space_parent_targets = (
                object_space_parent_targets
                if self._preserve_object_space_parent_reference_frame(tag_animation)
                else []
            )
            base_animation = None
            ...
            for tag_animation in tag_animations:
                ...
                match tag_animation.animation_type:
                    ...
                    case AnimationType.REPLACEMENT:
                        ...
                        correction_nodes = tag_animation.shared_element.SelectField("object-space parent nodes")
                        ...
                        offset_node_indices = set()
                        offset_nodes = tag_animation.shared_element.SelectField("object space offset nodes")
                        for node_element in offset_nodes.Elements:
                            n_index = node_element.Fields[0].Value
                            if n_index < 0 or n_index >= nodes_count or n_index in offset_node_indices:
                                continue
                            offset_node_indices.add(n_index)
                            a_node = blender_animation.animation_nodes.add()
                            a_node.name = node_names[n_index]
                            a_node.node_type = "object_space_offset_node"
                            blender_animation.active_animation_node_index = len(blender_animation.animation_nodes) - 1
